...
const ResetPassword = () => {
    ...
    const { register, formState: { errors }, handleSubmit, watch } = useFormContext<{ password: string, confirmPassword: string }>()
    ...
    const onSubmit: SubmitHandler<{ password: string, confirmPassword: string }> = async (formData: { password: string, confirmPassword: string }) => {
        if (isValidPasswordMatch && !loading) {
            setLoading(true)
            const { success } = await resetPassword(path.endpoints.auth.resetPassword, formData.password)
            setLoading(false)
    ...
    return (
        <>
          ...
          border-[#939393] shadow-md w-full ${errors.password
                ...
                >
                    <IoEyeOffOutline
                        className={`absolute left-3 top-1/2 -translate-y-1/2 
                  w-5 h-5 text-[#4B4B4B] 
                  group-focus-within:text-[#4A73EA]  ${errors.confirmPassword
                                ? "text-[#D00416] group-focus-within:text-[#D00416]"
                                : ""
                            }`}
                    />
          ...
          font-medium 
          placeholder:text-[#4B4B4B] !font-roboto text-[16px] 
          border-[#939393] shadow-md w-full ${errors.confirmPassword
                                ? "border-[#D00416] text-[#D00416] focus:border-[#D00416] placeholder:text-[#D00416] focus:placeholder:text-[#D00416]"
                                : "custom-shadow-input focus:text-[#4A73EA] focus:placeholder:text-[#4A73EA]"
                            }`}
