...
const OTPVerification: React.FC<OTPVerificationProps> = ({
  ...
  const timeLeftRef = useRef<number>(expiryMinutes * 60);
  const [displayTime, setDisplayTime] = useState<number>(timeLeftRef.current);

  // Validate OTP code
  useEffect(() => {
    setIsValid(
      otpCode.length === maxLength && REGEXP_ONLY_DIGITS_AND_CHARS.test(otpCode)
    );
  }, [otpCode, maxLength]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setDisplayTime(timeLeftRef.current); // UI-ni yenilə
    }, 1000);
  ...
  const handleResendCode = (): void => {
    if (!isLoading) {
      onResendCode();
      timeLeftRef.current = expiryMinutes * 60;
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        <Box sx={{ mt: 1, display: "flex", alignItems: "center", gap: 1 }}>
          <Button
            ...
            disabled={isLoading || timeLeftRef.current > 0}
            sx={{
              p: 0,
              m: 0,
              textTransform: "none",
      ...
      >
        <Button
          type="submit"
          disabled={!isValid || isLoading || timeLeftRef.current <= 0}
          fullWidth
          sx={{
